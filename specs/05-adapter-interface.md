...
- Claude Code：`agent.apiBaseUrl` → `app_settings.anthropicBaseUrl` → `process.env.ANTHROPIC_BASE_URL` → SDK 默认。非空时 ClaudeCodeAdapter 改用 `ANTHROPIC_AUTH_TOKEN` + `ANTHROPIC_BASE_URL` env 注入（详见 Spec 01 §agents）。
  - **例外**：`input.apiKey` 以 `sk-ant-oat` 开头（Claude Pro/Max 订阅 OAuth 令牌，`claude setup-token` 生成）时优先走 `CLAUDE_CODE_OAUTH_TOKEN` 注入并清空 `ANTHROPIC_API_KEY`，忽略 `apiBaseUrl`——这类令牌不能当 `x-api-key` 用，否则官方 API 报 `Invalid API key · Fix external API key`。
...
详见上方顶级章节「API key 解析（共四层）」。ClaudeCodeAdapter 视角（注入由 `buildSdkEnv` 实现）：
- 第 1-3 层由 `AgentRunner.buildAdapterInput` 解析后塞进 `AdapterInput.apiKey`，adapter 默认注入 `options.env.ANTHROPIC_API_KEY` 传 SDK 子进程
- `sk-ant-oat` 开头的 OAuth 订阅令牌改注入 `CLAUDE_CODE_OAUTH_TOKEN`（并清空 `ANTHROPIC_API_KEY`）
- 配了 `apiBaseUrl`（第三方网关）时改注入 `ANTHROPIC_AUTH_TOKEN` + `ANTHROPIC_BASE_URL`（并清空 `ANTHROPIC_API_KEY`）
