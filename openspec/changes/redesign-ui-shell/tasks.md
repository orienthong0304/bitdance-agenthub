...
- [ ] 3.1 会话头部（标题 / 成员 / 审批模式 toggle / 用量入口）对齐设计稿。
- [ ] 3.2 消息流：气泡 / 工具卡 / 产物卡 / 调度卡视觉对齐（紧凑字号 11-13px 体系）。
- [ ] 3.3 输入区：引用条 / 附件 / @提及 / 发送与中止按钮布局对齐。
- [ ] 3.4 全量 e2e 回归。

## 4. Phase D — Workspace Dock（右侧）

- [ ] 4.1 产物预览 dock 视觉对齐（空态「从左侧选择一个产物查看」、头部操作组）。
...
- [ ] 6.1 `specs/09-frontend-architecture.md` 同步应用壳结构与 token 约定。
- [ ] 6.2 OVERVIEW 功能矩阵 / 代码地图同步。
