...
| **CodexAdapter** | ⏳ | 待接入（优先 `@openai/codex-sdk`，CLI 仅 fallback） |
...
| LLM SDK | `@anthropic-ai/claude-agent-sdk`、`@anthropic-ai/sdk`、`openai`；CodexAdapter 待接入 `@openai/codex-sdk` |
...
- [ ] Codex adapter（官方 Codex SDK 集成；`codex exec` / MCP 仅作 fallback 或自动化参考；目前可用 `gpt-5-codex` 模型 + CustomAgent 走 OpenAI 协议替代）
