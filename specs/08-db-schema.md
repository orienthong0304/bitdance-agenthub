...
跨会话聚合的任务看板（openspec task-board，详见 Spec 01 Task 实体）。三种来源 `manual` / `dispatch` / `agent`；不与其它表建 FK 约束——会话 / 消息 / 产物被删除时任务保留，回链字段只是逻辑关联。
...
tasks {
  ...
  source              text NOT NULL      // 'manual' | 'dispatch' | 'agent'
  conversation_id     text               // 无 FK；来源会话删除后置空，任务保留
  message_id          text
  artifact_id         text
  dispatch_task_id    text               // dispatch 来源幂等键 `${runId}:${taskId}`
...
**索引**：
- `idx_tasks_status`：看板按状态分组查询
- `idx_tasks_dispatch`：`dispatch_task_id` 唯一索引（`WHERE dispatch_task_id IS NOT NULL`），供 `upsertDispatchTask` 幂等 upsert（重复批准 / replan 不会插入重复行）

**约束**：
- 无 CASCADE：会话 / 消息 / 产物删除不级联删除任务；前端渲染时 `conversationId` 找不到对应会话就隐藏跳转能力
- `syncDispatchTaskStatus` 是单向同步：dispatch 执行状态变化写回看板，看板编辑（手动切状态）不反向影响 dispatch
...
tasks ──(独立，无外键)── 跨会话聚合实体，conversation_id/message_id/artifact_id/dispatch_task_id 仅逻辑关联
```
